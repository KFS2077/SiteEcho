  ...
  if (!previewData) {
    return (
      ...
      >
        ...
        <p className="text-slate-500 dark:text-slate-400">Analyze a website to discover similar sites</p>
      </motion.div>
    )
  }
  ...
  return (
    ...
    >
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h3 className="text-xl font-bold text-slate-800 dark:text-slate-100">Discover Similar Sites</h3>
          <p className="text-sm text-slate-500 dark:text-slate-400">Based on: {new URL(previewData.url).hostname}</p>
        </div>
      </div>
                    ...
                    <span
                      ...
                      className={`${
                        searchTerm && keyword.toLowerCase().includes(searchTerm.toLowerCase())
                          ? "px-2 py-1 rounded-full text-xs font-medium bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-800"
                          : "px-2 py-1 rounded-full text-xs font-medium bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300"
                      }`}
                    >
                      {keyword}
                    </span>
      ...
      {filteredSites.length === 0 && (
        <div className="text-center py-8">
          <p className="text-slate-500 dark:text-slate-400">No similar websites found matching your search</p>
        </div>
      )}
    </motion.div>
  )
